...
class Command(BaseCommand):
    help = 'Add plants using direct file names for static/images path'

    def handle(self, *args, **options):
        # Clear existing plants
        Plant.objects.all().delete()
        
        # Sample plants data - storing only filename
        plants_data = [
            {
                'name': 'Oleander',
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Successfully added {len(plants_data)} plants using static/images path')
        )
